...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingDto save(BookingDtoCreate bookingDtoCreate, Long userBookerId) {
        ...
        request.setDescription("Запрос на бронирование вещи");
    ...
    public BookingDtoPatchApproved updateBookingStatus(Long bookingId, boolean approved, Long userOwnerId) {
        ...
        Long userIdByOwnerFromBooking = booking.getItem().getUser().getId(); // id владельца вещи из бронирования
        if (!userIdByOwnerFromBooking.equals(userOwnerId)) { // сравниваем id владельца вещи из бронирования с userOwnerId
            throw new ValidationException("Статус у бронирования может изменить только владелец.");
        }

        // в соответствии с параметром approved устанавливаем статус
        if (approved) {
            booking.setStatus(BookingStatusEnum.APPROVED); // бронирование имеет статус - подтвержден
        } else {
            booking.setStatus(BookingStatusEnum.REJECTED);// бронирование имеет статус - отклонен
        }

        Booking bookingSave = bookingRepository.save(booking);  // сохраняем
